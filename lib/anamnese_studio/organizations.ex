defmodule AnamneseStudio.Organizations do
  @moduledoc """
  The Organizations context.
  """

  import Ecto.Query, warn: false
  alias AnamneseStudio.Repo
  ...
  def subscribe_organizations(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(AnamneseStudio.PubSub, "user:#{key}:organizations")
  end

  defp broadcast_organization(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(AnamneseStudio.PubSub, "user:#{key}:organizations", message)
  end
  ...
  def get_organization!(%Scope{} = scope, id) do
    Repo.get_by!(Organization, id: id, owner_id: scope.user.id)
  end
  ...
  def update_organization(%Scope{} = scope, %Organization{} = organization, attrs) do
    true = organization.owner_id == scope.user.id
